...
class _CaisseScreenState extends State<CaisseScreen> {
  ...
  bool _onKey(KeyEvent event) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.enter && _ligneSelectionneeId != null) {
      _appliquerQteBuffer();
      return true;
    }
    return false;
  ...
  void _appendQte(String digit) {
    if (_ligneSelectionneeId == null) return;
    setState(() {
  ...
  void _appliquerQteBuffer() {
    ...
    final qte = int.tryParse(_qteBuffer) ?? 1;
    try {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('CAISSE', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 2)),
      ...
      body: isWide
          // ── PC / Tablette (≥800px) : layout côte à côte ──
          // Catalogue à gauche (62%), panier + numpad permanent à droite (38%)
          ? Row(children: [
              Expanded(flex: 5, child: _buildCatalogue()),
              Container(width: 1, color: Colors.grey.shade300),
              SizedBox(width: size.width * 0.38, child: _buildPanier()),
            ])
          // ── Mobile (<800px) : layout vertical ──
          // Catalogue en haut (55%), panier en bas (45%)
          // Le numpad s'affiche UNIQUEMENT si une ligne est sélectionnée
          : Column(children: [
              Expanded(flex: 55, child: _buildCatalogue()),
              Divider(height: 1, color: Colors.grey.shade400),
              Expanded(flex: 45, child: _buildPanier()),
            ]),
    );
  }
              ...
              : LayoutBuilder(builder: (_, constraints) {
                  ...
                  return GridView.builder(
                    ...
                    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                      crossAxisCount: cols, crossAxisSpacing: 8, mainAxisSpacing: 8, childAspectRatio: 1.05,
                    ),
  ...
  Widget _buildProduitCard(Produit p) {
    ...
    final enRupture = p.ingredients.any((ip) {
      ...
      return ing != null && ing.estImportant && ing.quantite < ip.quantiteUtilisee;
    });
    ...
    return GestureDetector(
      onTap: enRupture
          ...
          : () {
              try {
                prov.ajouterProduitNouvelLigne(p);
              } catch (e) {
                _showErreur(e.toString().replaceAll('Exception: ', ''));
              }
            },
      child: Container(
        decoration: BoxDecoration(
          ...
          border: enRupture ? Border.all(color: Colors.red.shade200) : null,
        ),
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.fastfood, size: 28, color: enRupture ? Colors.grey.shade400 : const Color(0xFF2D3561)),
              const SizedBox(height: 4),
              Text(p.nom, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis,
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12,
                    color: enRupture ? Colors.grey.shade500 : Colors.black87)),
              const SizedBox(height: 2),
              Text('${_fmt.format(p.prix)} DT',
                style: TextStyle(color: enRupture ? Colors.grey : const Color(0xFFE94560),
                    fontWeight: FontWeight.bold, fontSize: 11)),
              if (enRupture)
                const Text('Rupture', style: TextStyle(color: Colors.red, fontSize: 9)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildPanier() {
    ...
    // Sur PC/tablette : numpad toujours visible à droite
    // Sur mobile : numpad uniquement si une ligne est sélectionnée
    final showNumpad = isWide || _ligneSelectionneeId != null;
    ...
    return Container(
      ...
      child: Column(
        children: [
                ...
                ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        if (isWide) ...[
                          const SizedBox(height: 8),
                          Text('Sélectionnez une ligne pour modifier la quantité',
          ...
          // ── Numpad quantité ──
          // PC/tablette : toujours affiché
          // Mobile : uniquement si ligne sélectionnée
          if (showNumpad) _buildNumpadQuantite(isWide: isWide),
  ...
  Widget _buildLigneCommande(LigneCommande ligne) {
    ...
    return GestureDetector(
      onTap: () => setState(() {
        _ligneSelectionneeId = selected ? null : ligne.id;
        _qteBuffer = '';
      }),
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 150),
        decoration: BoxDecoration(
          color: selected ? const Color(0xFF2D3561).withValues(alpha: 0.06) : null,
          border: Border(
            left: BorderSide(color: selected ? const Color(0xFF2D3561) : Colors.transparent, width: 3),
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  ...
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13,
                      color: selected && _qteBuffer.isNotEmpty ? const Color(0xFFE94560) : null),
                ),
              ),
              _qteBtn(Icons.add, () {
                try {
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(ligne.produit.nom,
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
                    overflow: TextOverflow.ellipsis),
                  if (ligne.supplements.isNotEmpty)
                    Text('+ ${ligne.supplements.map((s) => s.nom).join(', ')}',
              ...
              Text('${_fmt.format(ligne.sousTotal)} DT',
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (_) => [
                  const PopupMenuItem(value: 'supplement',
                    child: ListTile(dense: true, leading: Icon(Icons.add_circle_outline), title: Text('Supplément'))),
                  const PopupMenuItem(value: 'commentaire',
                    child: ListTile(dense: true, leading: Icon(Icons.comment), title: Text('Commentaire'))),
                  const PopupMenuItem(value: 'supprimer',
                    child: ListTile(dense: true, leading: Icon(Icons.delete, color: Colors.red),
  ...
  Widget _buildNumpadQuantite({bool isWide = false}) {
    // Taille des boutons adaptée à la plateforme
    // PC/tablette : boutons plus grands car espace disponible
    // Mobile : boutons compacts pour ne pas déclencher l'overflow
    final btnHeight = isWide ? 44.0 : 30.0;
    final fontSize = isWide ? 18.0 : 15.0;
    final vPadding = isWide ? 10.0 : 6.0;

    // Message d'état selon la sélection
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          // Séparateur visible uniquement sur PC/tablette
          if (isWide) const Divider(height: 1, color: Color(0xFFE0E0E0)),
          ...
          Row(
            children: [
              Icon(
                hasSelection ? Icons.dialpad : Icons.touch_app,
                size: 14,
                color: labelColor,
              ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: col.map((k) {
                      final isErase = k == '⌫';
                      final isConfirm = k == '✓';
                      // Désactiver si pas de ligne sélectionnée
                      final enabled = hasSelection;
                      return Padding(
  ...
  Widget _buildTotalBar(AppProvider prov) {
    final total = prov.totalCommande;
    final tva = total - (total / 1.19);
    final ht = total / 1.19;
    final isWide = MediaQuery.of(context).size.width >= 800;

    // Taille adaptée : compact sur mobile pour éviter l'overflow
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                'HT: ${_fmt.format(ht)} DT',
                style: const TextStyle(color: Colors.white54, fontSize: 10),
              ),
              Text(
                'TVA 19%: ${_fmt.format(tva)} DT',
                style: const TextStyle(color: Colors.white54, fontSize: 10),
              ),
            ],
          ),
  ...
  void _handleLigneAction(String action, LigneCommande ligne) {
    ...
    switch (action) {
      case 'supprimer':
        ...
        if (_ligneSelectionneeId == ligne.id) setState(() => _ligneSelectionneeId = null);
        break;
      case 'supplement':
        ...
        content: SizedBox(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            TextField(controller: ctrl, maxLines: 2,
              decoration: const InputDecoration(labelText: 'Commentaire libre', border: OutlineInputBorder())),
            if (predefinis.isNotEmpty) ...[
              const SizedBox(height: 12),
              const Align(alignment: Alignment.centerLeft,
                child: Text('Rapide:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
              const SizedBox(height: 6),
              Wrap(spacing: 6, runSpacing: 4,
                children: predefinis.map((c) => ActionChip(
  ...
  void _confirmerAnnulation() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Voulez-vous vraiment annuler toute la commande en cours ?'),
        actions: [
